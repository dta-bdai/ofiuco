...
def parse_lock_file(data, platforms = None, generate_extras = True, extra_pip_args = []):
    _MARKERS = "markers = "
    _SOURCE_URL = "url = "
    _SOURCE_TYPE = "type = "
...
package(
  ...
  files = {{{files}
   }},{deps}{markers}{source_urls}{extra_index_urls}{extra_pip_args}{platforms}
...
""".format(
            ...
            extra_pip_args = "\n  extra_pip_args = [{}],".format(", ".join(["'{}'".format(arg) for arg in extra_pip_args])) if extra_pip_args else "",
...
def _poetry_parse_impl(rctx):
    ...
    host_cpu_arch = rctx.os.arch
    lock_file = Label(rctx.attr.lock_by_platform.get(host_cpu_arch, rctx.attr.lock))
    command = "import tomllib; print('{0}', 1); print('ok'); print(tomllib.load(open('{0}', 'rb')))".format(rctx.path(lock_file))
    exec_result = rctx.execute([interpreter, "-c", command], quiet = True)
    if exec_result.return_code:
        fail("Parsing {} failed with exit code {}\n{}\n".format(lock_file, exec_result.return_code, exec_result.stderr))
    ...
    lock_file_content = parse_lock_file(rctx.read(lock_file), rctx.attr.platforms, rctx.attr.generate_extras, rctx.attr.extra_pip_args)
...
poetry_parse = repository_rule(
    attrs = {
        ...
        "lock_by_platform": attr.string_keyed_label_dict(
            doc = """A dict mapping from target platform to corresponding lock file.
            Currently only support target platform with <cpu> type.""",
        ),
        ...
        "extra_pip_args": attr.string_list(
            doc = "Extra args for pip install package",
        ),
